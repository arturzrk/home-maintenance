...
scripts:
  sh: polaris agent check-prerequisites --json --require-tasks --include-tasks
  ps: polaris agent -Json -RequireTasks -IncludeTasks
---
## Model Guidance
...
- **constitution.md** – Project principles (from `.polaris/memory/constitution.md`)
...
**Before this**: `/polaris.tasks` created your task breakdown (tasks.md complete)
...
- Proceed to `/polaris.implement` when ready
- Or return to `/polaris.plan` or `/polaris.tasks` if major revisions needed
