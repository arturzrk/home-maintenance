...
**Telemetry**: Run: `polaris telemetry record standards --feature <slug> --phase start --agent claude`
...
**DAST**: Detect tooling via `detect_dast_tools()` from `specify_cli.assess.tooling_detection`. Supported: OWASP ZAP (.zap/, zap.yaml), Nuclei (nuclei-.polaris/templates/, .nuclei-config.yaml), Burp Suite (.burp/, burp-project.json). When not detected, include setup guidance (install, config, run, docs).
...
**Telemetry**: Run: `polaris telemetry record standards --feature <slug> --phase complete --agent claude`
