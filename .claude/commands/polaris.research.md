...
*Path: [.polaris/templates/commands/research.md](.polaris/templates/commands/research.md)*

**Telemetry**: Run: `polaris telemetry record research --feature <slug> --phase start --agent claude`
...
When you run `polaris research`, the following files are generated in your feature directory:
...
**Location**: All files go in `polaris-specs/001-feature-name/`

---

## Workflow Context

**Before this**: `/polaris.plan` calls this as "Phase 0" research phase
...
- Continue with `/polaris.plan` which uses your research to drive technical design
...
2. Run `polaris research` to generate the mission-specific research artifacts. (Add `--force` only when it is acceptable to overwrite existing drafts.)
...
5. Summarize open questions or risks at the bottom of `research.md`. These should feed directly into `/polaris.tasks` and future implementation prompts.

## Success Criteria

- `polaris-specs/<feature>/research.md` explains every major decision with references to evidence.
- `polaris-specs/<feature>/data-model.md` lists the entities and relationships needed for implementation.
...
**Telemetry**: Run: `polaris telemetry record research --feature <slug> --phase complete --agent claude`
